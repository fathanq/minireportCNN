...
train_data = train_data.shuffle(312) # if you want to shuffle your data
train_data = train_data.batch(batch_size)

# create testing Dataset and batch it
test_data = tf.data.Dataset.from_tensor_slices(test)
test_data = test_data.shuffle(104)
...
w3 = tf.get_variable("w3", shape=(M,52), initializer=tf.random_normal_initializer(0,0.01))
b3 = tf.get_variable("b3", shape=(1,52), initializer=tf.zeros_initializer())
...
#w5 = tf.get_variable("w5", shape=(O,52), initializer=tf.random_normal_initializer(0,0.01))
#b5 = tf.get_variable("b5", shape=(1,52), initializer=tf.zeros_initializer())
...
with tf.Session() as sess:
    ...
    loss_vector = np.arange(n_epochs)
    for i in range(n_epochs): 	
        ...
        loss_vector[i]= total_loss/n_epochs
    ...
    total_cmat = np.zeros((52,52),dtype="int32")
    try:
        while True:
